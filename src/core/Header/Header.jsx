        ...
        <div ref={menuRef} className={`w-full fixed top-0 z-20  transition-all duration-300 ${ isActive === "/" ? scrolled ? "bg-gradient-to-r from-[#E52020] to-[#A40000]"  : "bg-fixed" : `bg-white ${scrolled ? "shadow-md" : ""}` }`}>
            <div className="mx-auto w-full xl:max-w-[1450px] md:max-w-[920px] flex items-center justify-between px-3">
                ...
                <div className="lg:flex hidden items-center">
                    <ul className={`flex gap-18 h-full items-center justify-center roboto-slab-medium text-[20px] ${ isActive === "/" ? scrolled ? 'text-white ' : 'text-white' : "text-gray-700"}`}>
                        <li >
                            <Link to="/" >Home</Link>
                        </li>
                                    ...
                                    <ul className="p-2 text-[18px] max-w-none w-full whitespace-nowrap ">
                                        {Aboutus.map((link, index) => (
                                            <Link key={index} onClick={() => setDropdown1(false)} to={link.path} className="!w-full">
                                                <li className="px-4 py-2 text-gray-800 roboto-slab-regular  ">{link.name}</li>
                                            </Link>
                                        ))}
                                    </ul>
                                </div>
                                    ...
                                    <ul className="p-2 text-[18px] max-w-none w-full whitespace-nowrap">
                                        {AuthorDesk.map((link, index) => (
                                            <Link key={index} onClick={() => setDropdown2(false)} to={link.path} className="!w-full">
                                                <li className="px-4 py-2 text-gray-800 roboto-slab-regular  mt-1   ">{link.name}</li>
                                            </Link>
                                        ))}
                                    </ul>
                                </div>
                ...
                >
                    <RiMenu3Fill  className={`ml-15 text-3xl cursor-pointer duration-300 m-1.5 ${ isActive==="/" ? scrolled ? 'text-white ' : 'text-white' : "text-gray-900"} `} />
                </div>
            </div>
